...
from data_loader import get_pgl_loader
...
class Solver(object):
    ...
    def run(self, splits=5):
        ...
        for i in range(splits):
            self.train_loader = get_pgl_loader('train', 'TVSum', i)
            self.test_loader = get_pgl_loader('test', 'TVSum', i)
            self.train()
    ...
    def train(self):
        ...
        for epoch in tqdm(range(0, self.config.n_epochs)):
            ...
            for frame_features, target in tqdm(self.train_loader, desc='Batch', ncols=80):
    ...
    def test(self):
        ...
        loss_history = []
        f1s = []
        for frame_features, video_name, user_summary, gt_summary, cps, n_frames, nfps, positions in tqdm(self.test_loader, desc='Batch', ncols=80):
